...
shm_id1=shmget (SHMKEY1,sizeof(grammi),PERM|IPC_CREAT);		//same key with shared memory 1
	if(shm_id1==-1)
...
shm_id2=shmget (SHMKEY2,sizeof(int),PERM|IPC_CREAT);			//shared memory for counter Ci
	if(shm_id2==-1)
...
sem_id=semget(SHMKEY1,3,PERM | IPC_CREAT);		//create 3 semaphore for shared memory 1
...
if ((pids=calloc((N+1),sizeof(pid_t)))==NULL ){				//tha perasoume ta pid se array
	...
	pid=fork();
	pids[i]=pid;
	if(!pid)			//if child N Ci with single  father
	break;
	}
	
	if(!pid){	sleep(1);
			down(sem_id,1);			// down 2o semaphore only one Ci
			sleep(10);
			*count=*count+1;	
			if(*count==1)			//critical district only one Ci
			grammi2->c=N;
			...
			gettimeofday(&first, NULL);		//time response
			
			
			up(sem_id,0);				//unblock processes D
			down(sem_id,2);				//block processes C only D ->shared memory 1
	...
	for(l=0;l<N;l++){			//wait childrens to end 
		pids[l]=wait(&status);
		if(pids[l]!=-1)
